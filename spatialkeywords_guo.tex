...
which corresponds to the defination of \textsf{TYPE2} problem. When such circle
areas is determined the number of candidate objects can be greatly decreased
...
\If{mask == keywordsBM}{
	\If{selectCost+furthestDist<bestCost}{
		$bestCost \gets selectCost+furthestDist$\;
		$resultSet \gets selectedSet$\;
	}
	\Return $bestCost$\;
}
\If{$selectCost+furthestDist>bestCost$}{
...
\For{each i in candidateSet}{
	\If{$ (object[i].mask \& (keywordsBM $\^{}$ mask) ) = 0 $}{
		{\bf continue}\;
	}
	$increment \gets 0$\;
	\For{each j in selectedSet}{
		$distance \gets Dist(object[i],object[j])$\;
		\If{distance > increment}{
			$increment\gets distance$\;
		}
	}
	$candidate.inc\gets increment$\;
	$candidate.id\gets i$\;
	$candidateList\gets candidateList + candidate$\;
	sort $candidateList$\;
	\For{candidate in candidateList}{
		\If{candidate.inc<selectedCost}{
			$selectedSet.insert(candidate.id)$\;
			$mask\gets mask|object[candidate.id].mask)$\;
		}
		\Else{
			{\bf break}\;
		}
	...
	\For{candidate in candidateList}{
		$selectedSet.insert(candidate.id)$\;
		\If{candidate.inc<bestdist}{
			$localSearch(selectedSet,candidateSet,$\
			$max(selectedCost,candidate.inc),furthestDist,$\
			$mask|object[candidate.id].mask)$\;
		}
		$selectedSet.erase(candidate.id)$\;
	}
}
